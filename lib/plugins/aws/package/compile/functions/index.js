...
class AwsCompileFunctions {
  ...
  downloadS3Artifact(functionName) {
    const S3 = new AWS.S3({ region: this.options.region });
    ...
    if (match) {
      return new BbPromise((resolve, reject) => {
        ...
        source.pipe(sink);
        source.on('data', chunk => buffers.push(chunk));
        source.on('close', () => resolve(Buffer.concat(buffers)));
    ...
    return BbPromise.each(
      ...
      functionName => {
        return BbPromise.bind(this)
          .then(this.downloadS3Artifact(functionName))
          .then(this.compileFunction(functionName));
      });
  }
